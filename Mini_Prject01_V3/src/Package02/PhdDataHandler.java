...
public class PhdDataHandler extends DataHandler {

	public static void savePhdStudentsToFile(String filename, List<PhdStudent> students) {
		
	    try (BufferedWriter writer = new BufferedWriter(new FileWriter(filename))) {
	        for (PhdStudent student : students) {
	            // Line 1: Student information (id, first_name, last_name, Major, overall_gpa)
	            writer.write(student.getId() + "," + student.getFirst_name() + "," + student.getLast_name() + "," + student.getResearchTopic() + "," + student.getResearchYears() + "," + student.getGrades().getOverallgpa());
	            writer.newLine();
	...
	public static List<PhdStudent> loadPhdStudentsFromFile(String filename, List<String> dates) {
        List<PhdStudent> students = new ArrayList<>();
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(filename))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (parts.length >= 6) {
                    ...
                    String researchtopic = parts[3];     
                    int researchyears = Integer.parseInt(parts[4]);   
                    double overallGpa = Double.parseDouble(parts[5]);
                    
                    PhdStudent student = new PhdStudent(firstName, lastName, id, researchtopic , researchyears);
